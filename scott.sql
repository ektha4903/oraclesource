...
--데이터 사전 : DB를 구성하고 운영하는데 필요한 정보를 담는 특수한 테이블, 직접 접근은 불가하고 뷰를 통해서 접근
SELECT * FROM DICT;
...
INSERT INTO VM_EMP_READ VALUES(7903,'JAMES','MANAGER');
--인라인 뷰
SELECT ROWNUM,e.* from emp e;
--시퀀스 : 특정 규칙에 맞는 연속 숫자 생성(게시판 글 번호, 상품주문 번호 생성시 사용)
CREATE TABLE DEPT_SEQUENCE AS SELECT * FROM DEPT WHERE 1<>1;
SELECT * FROM DEPT_SEQUENCE;

CREATE SEQUENCE SEQ_DEPT_SEQUENCE -- 필수
INCREMENT BY 10 --기본값은 1        --여기 부터 선택구문
START WITH 10   --기본값은 1
MAXVALUE 90   --기본값은 10의 27승까지
MINVALUE 0    --시작값 이하, 최대값 미만 값(기본값은 1)
NOCYCLE       --시퀀스 생성값이 MAX 도달할 경우 반복여부
CACHE 2;      --시퀀스 생성할 번호를 메모리에 미리 할당해 놓은 숫자

INSERT INTO DEPT_SEQUENCE VALUES(SEQ_DEPT_SEQUENCE.NEXTVAL,'DATA','SEOUL');
--sequence SEQ_DEPT_SEQUENCE.NEXTVAL exceeds MAXVALUE and cannot be instantiated : 할당한 값보다 커지면 안됨
...
--다른 사용자에게 TEMP 테이블을 조회, 삽입할 수 있는 권한부여
...
INSERT INTO TABLE_NOTNULL(LOGIN_ID,LOGIN_PWD,TEL) VALUES('TEST_ID01',NULL,'010-1234-5678');
INSERT INTO TABLE_NOTNULL(LOGIN_ID,LOGIN_PWD,TEL) VALUES('TEST_ID01','TEST_PWD01',NULL);
INSERT INTO TABLE_NOTNULL(LOGIN_ID,LOGIN_PWD) VALUES('TEST_ID02','TEST_PWD02');

--제약 조건 확인
SELECT * FROM user_constraints;
--제약 조건 시 이름 지정
CREATE TABLE TABLE_NOTNULL2(LOGIN_ID VARCHAR2(20) CONSTRAINT TBL_NN2_LOGID_NN NOT NULL, 
                            LOGIN_PWD VARCHAR2(20) CONSTRAINT TBL_NN2_LOGPWD_NN NOT NULL, TEL VARCHAR2(20));
--이미 생성된 테이블에 제약조건 추가
ALTER TABLE TABLE_NOTNULL2 MODIFY(TEL NOT NULL);
ALTER TABLE TABLE_NOTNULL2 MODIFY(TEL CONSTRAINT TBL_NN2_TEL_NN NOT NULL); --이름 지정하면서 조건을 추가할 때
--이미 생성된 제약 조건명 추가(제약 조건명 포함)
...
CREATE TABLE TABLE_UNIQUE(LOGIN_ID VARCHAR2(20) CONSTRAINT TBL_UNQ_LGNID_UNQ UNIQUE,
                          LOGIN_PWD VARCHAR2(20) CONSTRAINT TBL_UNQ_LGNPWD_UNQ NOT NULL, TEL VARCHAR2(20));

--PRIMARY KEY(기본키) : 유일하게 하나만 있는 값
CREATE TABLE TABLE_PK(LOGIN_ID VARCHAR2(20) PRIMARY KEY,
                          LOGIN_PWD VARCHAR2(20) NOT NULL, TEL VARCHAR2(20));
DESC TABLE_PK;
INSERT INTO TABLE_PK(LOGIN_ID,LOGIN_PWD,TEL) VALUES('HONG123','HONG123','010-1234-5678');
--unique constraint (SCOTT.SYS_C007038) violated
INSERT INTO TABLE_PK(LOGIN_ID,LOGIN_PWD,TEL) VALUES('HONG123','HONG123','010-1234-5678');

--이름 지정
CREATE TABLE TABLE_PK2(LOGIN_ID VARCHAR2(20 )CONSTRAINT TBL_PK_LGNID_PK PRIMARY KEY,
                          LOGIN_PWD VARCHAR2(20) CONSTRAINT TBL_PK_LGNPWD_PK NOT NULL, TEL VARCHAR2(20));

--제약 조선 모아서 주기 (NOT NULL은 제외)
CREATE TABLE TABLE_CONSTRAINT(ID NUMBER(4),
                              NAME VARCHAR2(10),
                              ADDR VARCHAR2(20),
                              TEL VARCHAR2(20),
...
INSERT INTO EMP(EMPNO,ENAME,JOB,MGR,HIREDATE,SAL,COMM,DEPTNO) VALUES(9999,'홍길동','CLERK','7788',SYSDATE,1200,NULL,50);
...
CREATE TABLE EMP_FK(EMPNO NUMBER(4) CONSTRAINT EMPFK_EMPNO_PK PRIMARY KEY,
                    ...
                    SAL NUMBER(7,2),
                    COMM NUMBER(7,2),
                    DEPTNO NUMBER(2) CONSTRAINT EMPFK_DEPTNO_KF REFERENCES DEPT_FK(DEPTNO));

--외래키 지정시 부모 테이블에 먼저 데이터 입력
INSERT INTO DEPT_FK VALUES(50,'DATABASE','SEOUL');
INSERT INTO EMP_FK(EMPNO,ENAME,JOB,MGR,HIREDATE,SAL,COMM,DEPTNO) VALUES(9999,'홍길동','CLERK','7788',SYSDATE,1200,NULL,50);

--외래 키 지정시 데이터를 삭제한다면 참조하는 자식 테이블의 데이터를 먼저 삭제
--integrity constraint (SCOTT.EMPFK_DEPTNO_KF) violated - child record found
...
CREATE TABLE EMP_FK(EMPNO NUMBER(4) CONSTRAINT EMPFK_EMPNO_PK PRIMARY KEY,
                    ...
                    COMM NUMBER(7,2),
                    DEPTNO NUMBER(2) CONSTRAINT EMPFK_DEPTNO_KF REFERENCES DEPT_FK(DEPTNO)
                    ON DELETE CASCADE); --참조하는 데이터도 같이 삭제
...
CREATE TABLE TABLE_DEFAULT(LOGIN_ID VARCHAR2(20) CONSTRAINT TBLCK2_LOGINID_PK PRIMARY KEY,
                           ...
                           TEL VARCHAR2(20));
INSERT INTO TABLE_DEFAULT(LOGIN_ID, TEL) VALUES('TEST','010-1234-5678');
SELECT * FROM TABLE_DEFAULT;

--Q) DEPT_CONST 테이블 작성하기
--열 이름 DEPTNO 정수형 숫자(2), PRIMARY KEY, 제약 조건명 : DEPTCONST_DEPTNO_PK
--열 이름 DNAME 가변형 문자열(14), UNIQUE, 제약 조건명 : DEPTCONST_DNAME_UNQ
--열 이름 LOC 가변형 문자열(13), NOT NULL, 제약 조건명 : DEPTCONST_LOC_NN
CREATE TABLE DEPT_CONST(DEPTNO NUMBER(2) CONSTRAINT DEPTCONST_DEPTNO_PK PRIMARY KEY,
...
CREATE TABLE EMP_CONST(EMPNO NUMBER(4) CONSTRAINT EMPCONST_EMPNO_PK PRIMARY KEY,
                       ENAME VARCHAR2(14) CONSTRAINT EMPCONST_ENAME_NN NOT NULL,
                       JOB VARCHAR2(9),
